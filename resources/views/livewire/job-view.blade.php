<div class="min-h-screen w-[100vw] flex items-center justify-center bg-gray-200  text-white p-4">
    <div class="bg-white backdrop-blur-lg p-6 rounded-xl shadow-lg w-full max-w-lg">
        ...
        <p class="text-black/90 mb-2"><strong>Description:</strong> {{ $job->description }}</p>
        ...
        <div class="w-full flex justify-between px-8">
            <a href="{{ route('home') }}"
                class="inline-block mt-4 px-4 py-2 bg-black/30 font-semibold text-white/90 rounded hover:bg-green-500 hover:text-white transition duration-300 ease-in-out">
                &larr; Back
            </a>
            <a href="{{ route('jobs-apply', $job->id) }}"
                class="inline-block mt-4 px-4 py-2 bg-green-500 font-semibold text-white rounded hover:bg-white hover:ring hover:ring-black hover:text-green-500 transition duration-300 ease-in-out">
