    ...
    <div class="bg-white shadow-lg rounded-2xl p-6 max-w-5xl mx-auto">
        ...
        <div class="flex mx-auto justify-between">
            <div class="w-[3/4] flex items-center space-x-4 mb-2">
                ...
                @else
                    <img src="{{Auth::user()->image}}" class="w-32 h-32 rounded-full border-4 border-white shadow"
                        alt="Profile Photo">
                @endif
                <div>
            ...
            <div class="flex justify-between w-full px-8 gap-8">
                <div class="w-[60%] py-2">
                    <div>
                        ...
                        @else
                            <h2 class="px-2 py-2 text-l font-bold text-gray-500">{{ Auth::user()->company?->name }}</h2>
                        @endif
                    </div>
                    <div>
                        ...
                        @else
                            <h2 class="px-2 py-2 text-l font-bold text-gray-500">{{ Auth::user()->company?->location }}</h2>
                        @endif
                    </div>
                    <div>
