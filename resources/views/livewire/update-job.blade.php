        ...
        <div class="z-10 bg-white/10 backdrop-blur-md text-white rounded-xl shadow-2xl p-8 w-[400px]">
            ...
            <div class="mb-4 flex justify-center">
                <div class="w-32 h-32 relative rounded-full border-4 border-white shadow mb-6">
                    ...
                    <input type="file" wire:model="image" wire:keydown.enter="updateData"
            ...
            <x-form.input-field name="name" placeholder="Job Title" wire:keydown.enter="updateData"
                wire:blur="updateData">
            ...
            <x-form.input-field name="salary" placeholder="Salary" wire:keydown.enter="updateData"
                wire:blur="updateData">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                    stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="mb-4">
                <textarea wire:model="description" placeholder="Job Description"
                    ...
                    rows="4" wire:blur="updateData">{{ $description }}</textarea>
                @error('description') <span class="text-red-300 text-sm">{{ $message }}</span> @enderror
            </div>

            <div class="mb-4">
                <select wire:model="tag_id" wire:blur="updateData" wire:keydown.enter="updateData"
            ...
            @if (session()->has('success'))
                <div class="w-full flex justify-center text-green-400 pt-2">{{ session('success') }}</div>
            @endif
        </div>
